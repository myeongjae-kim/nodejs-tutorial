...
아래처럼 `DiConfig`라는 인터페이스를 만들어 사용합니다. 개별 도메인마다 `DiConfig` 인터페이스를 구현하는 클래스를 만들어서 의존성 주입 설정을 합니다.
